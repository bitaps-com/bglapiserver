...
def test_get_block_transactions_extended(conf):
    ...
    r = requests.get("https://api.bitaps.com/bgl/v1/blockchain/block/30075?"
                     "block_statistic=1&transactions=1&mode=verbose")
    assert r.status_code == 200
    br = r.json()["data"]["transactions"]["list"]

    r = requests.get(conf["base_url"] + "/rest/block/transactions/30075?mode=verbose")
    assert r.status_code == 200
    tms = time.time()
    d = r.json()["data"]
    ...
    r = requests.get("https://api.bitaps.com/bgl/v1/blockchain/block/500075?"
                     "block_statistic=1&transactions=1&mode=verbose&page=5")
    assert r.status_code == 200
    br = r.json()["data"]["transactions"]["list"]

    r = requests.get(conf["base_url"] + "/rest/block/transactions/30075?mode=verbose&page=5")
    assert r.status_code == 200
    tms = time.time()
    d = r.json()["data"]
    ...
    r = requests.get(conf["base_url"] + "/rest/block/transactions/30075?mode=brief")
    assert r.status_code == 200
    tms = time.time()
    d = r.json()["data"]
