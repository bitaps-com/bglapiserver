...
def test_address_state_extended(conf):
    r = requests.get(conf["base_url"] + "/rest/address/state/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r")
...
def test_address_transactions(conf):
    r = requests.get(conf["base_url"] + "/rest/address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r")
    assert r.status_code == 200
    l = r.json()["data"]

    r = requests.get("https://api.bitaps.com/bgl/v1/blockchain/"
                     "address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r")
    assert r.status_code == 200
    r = r.json()["data"]
    ...
    for page in range(10):
        r = requests.get(conf["base_url"] + "/rest/address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?page=%s" % page)
        assert r.status_code == 200
        l = r.json()["data"]

        r = requests.get("https://api.bitaps.com/bgl/v1/blockchain/"
                         "address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?page=%s" % page)
        assert r.status_code == 200
        r = r.json()["data"]
    ...
    r = requests.get(conf["base_url"] + "/rest/address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?order=asc")
    assert r.status_code == 200
    l = r.json()["data"]

    r = requests.get("https://api.bitaps.com/bgl/v1/blockchain/"
                     "address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?order=asc")
    assert r.status_code == 200
    r = r.json()["data"]
    ...
    r = requests.get(conf["base_url"] + "/rest/address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?mode=verbose")
    assert r.status_code == 200
    l = r.json()["data"]

    r = requests.get("https://api.bitaps.com/bgl/v1/blockchain/"
                     "address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?mode=verbose")
    assert r.status_code == 200
    r = r.json()["data"]
    ...
    r = requests.get(conf["base_url"] + "/rest/address/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?from_block32292")
    assert r.status_code == 200
    l = r.json()["data"]
...
def test_address_unconfirmed_transactions(conf):
    r = requests.get(conf["base_url"] + "/rest/address/unconfirmed/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r")
    assert r.status_code == 200
    l = r.json()["data"]

    r = requests.get("https://api.bitaps.com/c/v1/blockchain/"
                     "address/unconfirmed/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r")
    assert r.status_code == 200
    r = r.json()["data"]
    for i in range(len(r["list"])):
        assert l["list"][i]["txId"] == r["list"][i]["txId"]
    ...
    r = requests.get(conf["base_url"] + "/rest/address/unconfirmed/transactions/"
                                        "bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?mode=verbose")
    assert r.status_code == 200
    l = r.json()["data"]

    r = requests.get("https://api.bitaps.com/bgl/v1/blockchain/"
                     "address/unconfirmed/transactions/bgl1qs6pce5xnxa58pthahz977m2hxpxyehthd2ax7r?mode=verbose")
    assert r.status_code == 200
    r = r.json()["data"]
    for i in range(len(r["list"])):
        assert l["list"][i]["txId"] == r["list"][i]["txId"]
