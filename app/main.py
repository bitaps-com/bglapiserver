...
import gzip
import pickle
from pybgl import Connector, encode_gcs, int_to_var_int, ripemd160, double_sha256, sha256, sha3_256
from pybgl import Transaction, MRU, bytes_to_int
from pybgl import map_into_range, siphash, target_to_difficulty, bits_to_target, int_to_bytes
from pybgl import s2rh, rh2s, merkle_tree, merkle_proof, parse_script, SCRIPT_N_TYPES
...
class App:
    def __init__(self, loop, logger, connector_logger, config):
        ...
        setproctitle('bglapi engine')
        self.log.info("BGLAPI server starting ...")
    ...
    async def start(self, config, connector_logger):
        ...
        try:
            ...
            self.connector =  Connector(config["CONNECTOR"]["rpc"],
                                        ...
                                        app_proc_title="bglapi engine connector")
    ...
    async def new_block_handler(self, block, conn):
        try:
            ...
            if self.blocks_data:
                ...
                print('records',[(block["height"],s2rh(block["hash"]),block["hash"],
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="BGLAPI server v 0.0.1")
